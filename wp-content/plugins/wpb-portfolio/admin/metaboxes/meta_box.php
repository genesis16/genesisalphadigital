 ...
 * WPB Metabox 
 * @v 1.00
 * Custom field added by WpBean: Gallery
 */


if( !function_exists('wpb_fp_custom_meta_box') ){

	add_action( 'init', 'wpb_fp_custom_meta_box' );
	function wpb_fp_custom_meta_box(){
		...
		function wpb_fp_custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
			if ( ! ( $field || is_array( $field ) ) )
				return;
			
			// get field data
			$type 				= isset( $field['type'] ) ? $field['type'] : null;
			$select_button 		= isset( $field['select_button'] ) ? $field['select_button'] : null;
			...
			switch( $type ) {
				...
				case 'shortcode':
					...
					$shortcode_value = '['.$default_value.' id="'.$post->ID.'"]';
					echo '<input type="text" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $shortcode_value ) .'" class="regular-text the-shortcode" size="30" readonly="readonly"/>
							<br />' . $desc;
				break;			
				case 'url':
				...
				break;
				case 'number':
					echo '<input type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $meta . '" class="regular-text" size="30" />
							<br />' . $desc;
				break;
				// textarea
				case 'textarea':
				...
				case 'select':
				case 'chosen':
					echo '<select class="wpb-fp-meta-select'. esc_attr( $type == 'chosen' ? ' chosen' : '') . esc_attr( $select_button == true ? ' wpb-fp-select-buttons' : '') .'" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"'. ( isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' ) . '>';
				...
				// slider
				case 'slider':
				$value = $meta != '' ? intval( $meta ) : $default_value;
				...
				// image
				case 'image':
					$image = WPB_FP_CUSTOM_METABOXES_DIR . '/images/image.png';	
				...
				case 'gallery':
					...
					echo '<div class="wpb-field-gallery">';

						$value  =  $meta;
						$hidden = ( empty( $value ) ) ? ' hidden' : '';
						$add    = esc_html__( 'Add Gallery', WPB_FP_TEXTDOMAIN );
					    $edit   = esc_html__( 'Edit Gallery', WPB_FP_TEXTDOMAIN );
					    $clear  = esc_html__( 'Clear', WPB_FP_TEXTDOMAIN );
					    ...
					    echo '<a href="#" class="button button-primary wpb-add">'. $add .'</a>';
					    echo '<a href="#" class="button wpb-edit'.$hidden.'">'. $edit .'</a>';
					    echo '<a href="#" class="button wpb-warning-primary wpb-remove'.$hidden.'">'. $clear .'</a>';
					    echo '<input type="text" name="' . esc_attr( $name ) . '" value="'. $value .'"/>';
					    echo '<br clear="all" />' . $desc;
				...
				case 'repeatable':
					...
					foreach ( $meta as $key => $row ) {
						echo '<tr class="wpb_fp_repeatable_item_'. $key .'">
						<td>';
						foreach ( $repeatable_fields as $repeatable_field ) {
							...
							echo '<div class="wpb_fp_repeatable_item_field">';
							echo '<label>' . $repeatable_field['label']  . '</label><p>';
							echo wpb_fp_custom_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
							echo '</p>';
							echo '</div>';
						} // end each field
						echo '</td><td><a class="meta_box_repeatable_remove" href="#" title="'. esc_html__( 'Remove this section', WPB_FP_TEXTDOMAIN ) .'"><span class="dashicons dashicons-no"></span></a></td></tr>';
						$i++;
					} // end each row
					echo '</tbody>';
					echo '
						<tfoot>
							<tr>
								<th><a class="meta_box_repeatable_add button" href="#">'. esc_html__( 'Add New', WPB_FP_TEXTDOMAIN ) .'</a></th>
							</tr>
						</tfoot>';
					echo '</table>
						' . $desc;
		 ...
		 */
		function wpb_fp_meta_box_find_field_type( $needle, $haystack ) {
			foreach ( $haystack as $h )
				if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
					return wpb_fp_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
		 ...
		 * @return	bool				whether or not the type is in the provided array
		 */
		function wpb_fp_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
			foreach ( $haystack as $h )
		 ...
		 */
		function wpb_fp_meta_box_santitize_boolean( $string ) {
		 ...
		 */
		function wpb_fp_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
			switch ( $function ) {
				case 'no':
					return $string;
				case 'intval':
		 ...
		 */
		function wpb_fp_meta_box_array_map_r( $func, $meta, $sanitizer ) {
			...
			foreach( $meta as $key => $array ) {
				...
				foreach( $array as $arrayKey => $arrayValue )
					if ( is_array( $arrayValue ) )
						$array[$arrayKey] = wpb_fp_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
		 ...
		 */
		class WPB_FP_Custom_Add_Meta_Box {
		    ...
		    public function __construct( $id, $title, $fields, $page, $context ) {
				$this->id = $id;
				$this->title = $title;
				$this->fields = $fields;
				$this->page = $page;
			...
			function admin_enqueue_scripts() {
				...
				if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
					// js
					$deps = array( 'jquery' );
					if ( wpb_fp_meta_box_find_field_type( 'date', $this->fields ) )
						$deps[] = 'jquery-ui-datepicker';
					if ( wpb_fp_meta_box_find_field_type( 'slider', $this->fields ) )
						$deps[] = 'jquery-ui-slider';
					if ( wpb_fp_meta_box_find_field_type( 'color', $this->fields ) )
						$deps[] = 'farbtastic';
					if ( in_array( true, array(
						wpb_fp_meta_box_find_field_type( 'chosen', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'post_chosen', $this->fields )
					) ) ) {
						wp_register_script( 'chosen', WPB_FP_CUSTOM_METABOXES_DIR . '/js/chosen.js', array( 'jquery' ) );
						$deps[] = 'chosen';
						wp_enqueue_style( 'chosen', WPB_FP_CUSTOM_METABOXES_DIR . '/css/chosen.css' );
					}
					if ( in_array( true, array( 
						wpb_fp_meta_box_find_field_type( 'date', $this->fields ), 
						wpb_fp_meta_box_find_field_type( 'slider', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'color', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'chosen', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'post_chosen', $this->fields ),
						wpb_fp_meta_box_find_repeatable( 'repeatable', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'image', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'gallery', $this->fields ),
						wpb_fp_meta_box_find_field_type( 'file', $this->fields )
					) ) )
						wp_enqueue_script( 'meta_box', WPB_FP_CUSTOM_METABOXES_DIR . '/js/scripts.js', $deps );
					
					// css
					$deps = array();
					wp_register_style( 'jqueryui', WPB_FP_CUSTOM_METABOXES_DIR . '/css/jqueryui.css' );
					if ( wpb_fp_meta_box_find_field_type( 'date', $this->fields ) || wpb_fp_meta_box_find_field_type( 'slider', $this->fields ) )
						$deps[] = 'jqueryui';
					if ( wpb_fp_meta_box_find_field_type( 'color', $this->fields ) )
						$deps[] = 'farbtastic';
					wp_enqueue_style( 'meta_box', WPB_FP_CUSTOM_METABOXES_DIR . '/css/meta_box.css', $deps );
				}
			}
			 ...
			 */
			function admin_head() {
				if ( in_array( get_post_type(), $this->page ) && ( wpb_fp_meta_box_find_field_type( 'date', $this->fields ) || wpb_fp_meta_box_find_field_type( 'slider', $this->fields ) ) ) {
			...
			function add_box() {
				...
				foreach ( $this->page as $page ) {
					add_meta_box( $this->id, $this->title, array( $this, 'meta_box_callback' ), $page, $this->context, 'high' );
				}
			}
				...
				foreach ( $this->fields as $field) {
					if ( $field['type'] == 'section' ) {
						...
						echo '<tr class="'.$field['id'].'">
								...
								echo wpb_fp_custom_meta_box_field( $field, $meta );
				...
				foreach ( $this->fields as $field ) {
					...
					if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
						...
						if ( isset( $new ) && '' == $new && $old ) {
							...
							if ( is_array( $new ) )
								$new = wpb_fp_meta_box_array_map_r( 'wpb_fp_meta_box_sanitize', $new, $sanitizer );
							else
								$new = wpb_fp_meta_box_sanitize( $new, $sanitizer );
