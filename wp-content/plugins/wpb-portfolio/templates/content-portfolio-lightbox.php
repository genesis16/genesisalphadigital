<?php
	...
	if( get_post_type() == 'wpb_fp_portfolio' ){
		$short_description 				= get_post_meta( $id, 'wpb_fp_portfolio_short_description', true );
		$external_url_btn_text 			= wpb_fp_get_option( 'wpb_fp_view_portfolio_btn_text_', 'wpb_fp_advanced', esc_html__( 'View Portfolio', WPB_FP_TEXTDOMAIN ) );
	}else{
		$short_description 				= get_the_excerpt();
		$external_url_btn_text 			= apply_filters( 'wpb_fp_quickview_details_btn_text', esc_html__( 'More Details', WPB_FP_TEXTDOMAIN ) );
	}


	if( !array_key_exists('title', $quickview_content_settings) && !array_key_exists('content', $quickview_content_settings) && !array_key_exists('button', $quickview_content_settings) ){
		$column = '';
		$row    = 'wpb_fp_wrapper';
	}else{
...
<div class="wpb_fp_quick_view_area wpb_fp_quick_view_<?php echo esc_attr( $quickview_layout ); ?>">
	<div class="<?php echo esc_attr( $row ); ?>">
		<?php if(array_key_exists('image', $quickview_content_settings)): ?>
			<div class="wpb_fp_quick_view_img <?php echo esc_attr( $images ? 'wpb_fp_has_gallery' : 'wpb_fp_no_gallery' );?> <?php echo ( $gallery_feature_image == 'on' ? 'wpb_fp_gallery_has_feature_image' : 'wpb_fp_gallery_disable_feature_image' );?> <?php echo esc_attr(apply_filters( 'wpb_fp_quick_view_img_column', $column )); ?>">
		...
		<?php if( array_key_exists('title', $quickview_content_settings) || array_key_exists('content', $quickview_content_settings) || array_key_exists('button', $quickview_content_settings) ): ?>
			<div class="wpb_fp_quick_view_content <?php echo esc_attr(apply_filters( 'wpb_fp_quick_view_content_column', $column )); ?>">
