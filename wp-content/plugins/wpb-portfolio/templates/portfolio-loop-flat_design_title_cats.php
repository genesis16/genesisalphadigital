...
if( $wpb_fp_link_full_grid_type == 'details_page' ){
	$link_full_grid 			= '<a '.$portfolio_link_target.' href="'. esc_url( $portfolio_permalink ) .'"><i class="wpbfpicons-link"></i></a>';
} elseif( $wpb_fp_link_full_grid_type == 'quickview_popup' ){
	if( $quickview_type == 'image_content' ){
		$link_full_grid = '<a data-post-id="' . esc_attr( $post->ID ) . '" class="wpb_fp_preview open-popup-link wpb-fp-open-popup-link-gallery" href="#" data-mfp-src="#'. esc_attr( $wpb_fp_quickview_id ) .'" data-effect="'. esc_attr( $wpb_fp_popup_effect ) .'">'. apply_filters( 'wpb_fp_quickview_icon', '<i class="wpbfpicons-eye"></i>' ) .'</a>';
	}else{
		$link_full_grid = '<a class="wpb_fp_image_lightbox" href="#" data-mfp-src="'. esc_url( wp_get_attachment_url( get_post_thumbnail_id(), 'full' ) ) .'">'. apply_filters( 'wpb_fp_quickview_icon', '<i class="wpbfpicons-eye"></i>' ) .'</a>';
	}
}
...
<div <?php post_class( $item_classes ) ?>>
	...
	<div class="portfolio_item">
		...
		<div class="portfolio_thumbnail wpb-fp-loader-wrapper">
