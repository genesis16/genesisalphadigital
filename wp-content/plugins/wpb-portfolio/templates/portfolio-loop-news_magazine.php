	...
	<div class="wpb-fp-portfolio-item-inner wpb-fp-loader-wrapper">
		...
		<div class="wpb-fp-portfolio-item-details">
			...
			<div class="wpb-fp-portfolio-read-more">
				<?php 
					if( $wpb_fp_link_full_grid_type == 'details_page' ){
						echo '<a class="wpb-fp-portfolio-read-more-link" '.$portfolio_link_target.' href="'. esc_url( $portfolio_permalink ) .'">'. apply_filters( 'wpb_fp_read_more_text', esc_html__( 'Read More', WPB_FP_TEXTDOMAIN ) ) .'</a>';
					} elseif( $wpb_fp_link_full_grid_type == 'quickview_popup' ){
						if( $quickview_type == 'image_content' ){
							echo '<a data-post-id="' . esc_attr( $post->ID ) . '" class="wpb-fp-portfolio-read-more-link wpb_fp_preview open-popup-link wpb-fp-open-popup-link-gallery" href="#" data-mfp-src="#'. esc_attr( $wpb_fp_quickview_id ) .'" data-effect="'. esc_attr( $wpb_fp_popup_effect ) .'">'. apply_filters( 'wpb_fp_read_more_text', esc_html__( 'Read More', WPB_FP_TEXTDOMAIN ) ) .'</a>';
						}else{
							echo '<a class="wpb-fp-portfolio-read-more-link wpb_fp_image_lightbox" href="#" data-mfp-src="'. esc_url( wp_get_attachment_url( get_post_thumbnail_id(), 'full' ) ) .'">'. apply_filters( 'wpb_fp_read_more_text', esc_html__( 'Read More', WPB_FP_TEXTDOMAIN ) ) .'</a>';
						}
					}
				?>
			</div>
