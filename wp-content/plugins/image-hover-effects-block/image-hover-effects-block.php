...
function create_image_hover_effects_block() {
    ...
    wp_register_script('iheg-image-hover', IHEG_URL . 'build/index.js', $asset_file['dependencies'], filemtime(IHEG_PATH . 'build/index.js'));
    wp_register_style('iheg-image-hover', IHEG_URL . 'assets/css/style.css', array(), filemtime(IHEG_PATH . 'assets/css/style.css'));
...
function iheg_render_callback($attributes){
    ...
    if($font_handle && $font_handle != 'inherit'){
        ...
        wp_enqueue_style( 'iheg-google-font-' . $font_handle, 'https://fonts.googleapis.com/css?family=' . $font_family . ':' . $attributes['title_font_weight'] . '&display=swap', array(), IHEG_VERSION );
    }
    ...
    if($font_handle && $font_handle != 'inherit'){
        $font_family = esc_attr( str_replace( '+', ' ', $font_handle ));
        $font_handle = str_replace( ' ', '-', strtolower( $font_handle ));
        wp_enqueue_style( 'iheg-google-font-' . $font_handle, 'https://fonts.googleapis.com/css?family=' . $font_family . ':' . $attributes['desc_font_weight'] . '&display=swap', array(), IHEG_VERSION );
    }
