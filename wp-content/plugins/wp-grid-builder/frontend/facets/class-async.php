...
class Async {
	...
	public function get_facet_items( $facet, $filtered = true ) {
		...
		$search_value = wp_specialchars_decode( $facet['search'], ENT_QUOTES );
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT facet_name, facet_value, facet_parent, COUNT(DISTINCT object_id) AS count
	...
	public function normalize( $facet, $items ) {
		...
		return array_map(
			function( $item ) use ( $count ) {
				...
				$item->count        = $count ? (int) $item->count : 0;
