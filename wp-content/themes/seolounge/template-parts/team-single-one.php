        ...
        <div class="row">
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                ...
                <div class="team-profilebuzz-contactcard">
                    <h2><?php the_title(); ?></h2>
                    <?php $terms = get_the_terms( get_the_ID(), 'profession' );
                    if ( ! empty( $terms ) ) {
                    ...
                    <hr>
                    <h3><?php echo esc_html__( 'Qualification', 'seolounge' ); ?></h3>
                    <?php if ( get_post_meta( get_the_ID(), 'qualification', true ) ) { ?>
			            <p><?php echo esc_html( get_post_meta( get_the_ID(), 'qualification', true ) ); ?></p>
			        <?php } ?>
                    <h3><?php echo esc_html__( 'Contact Info', 'seolounge' ); ?></h3>
                    <!-- contact -->
    			    <ul class="contact">
    			        <?php if ( get_post_meta( get_the_ID(), 'phone', true ) ) { ?>
    			            <li class="phone"><i class="fa fa-phone"></i> <a href="tel:<?php echo esc_html( get_post_meta( get_the_ID(), 'phone', true ) ); ?>" data-toggle="tooltip" data-placement="bottom" title="<?php echo esc_attr__( 'Make a Call', 'seolounge' ); ?>"><?php echo esc_html( get_post_meta( get_the_ID(), 'phone', true ) ); ?></a></li>
    			        <?php } ?>
    			        <?php if ( get_post_meta( get_the_ID(), 'email', true ) ) { ?>
    			            <li class="email"><i class="fa fa-envelope"></i> <a href="mailto:<?php echo esc_html( get_post_meta( get_the_ID(), 'email', true ) ); ?>" data-toggle="tooltip" data-placement="bottom" title="<?php echo esc_attr__( 'Send an Email', 'seolounge' ); ?>"><?php echo esc_html( get_post_meta( get_the_ID(), 'email', true ) ); ?></a></li>
    			        <?php } ?>
    			    </ul>
    			    <!-- contact -->
    			    <!-- social -->
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            <tbody>
                               ...
                               ?>
                                <tr>
                                    <td><?php if($field['TitleItem'] != '') echo esc_attr( $field['TitleItem'] ); ?></td>
                                    <td><?php if ($field['TitleDescription'] != '') echo esc_attr( $field['TitleDescription'] ); ?></td>
                                </tr>
                               <?php } ?>
                            </tbody>
                        </table>
