 ...
 * Studio Pro Theme
 *
 * This file adds the hero section to the child Theme.
 *
...
add_filter( 'genesis_attr_entry', 'studio_entry_attr' );
/**
 ...
 * @return array
 */
function studio_entry_attr( $atts ) {
...
add_action( 'genesis_before', 'studio_hero_section_setup' );
/**
 * Set up hero section.
 *
 ...
 * @return void
 */
function studio_hero_section_setup() {
	...
	add_action( 'studio_hero_section', 'genesis_do_posts_page_heading' );
	add_action( 'studio_hero_section', 'genesis_do_date_archive_title' );
	add_action( 'studio_hero_section', 'genesis_do_taxonomy_title_description' );
	add_action( 'studio_hero_section', 'genesis_do_author_title_description' );
	add_action( 'studio_hero_section', 'genesis_do_cpt_archive_title_description' );
...
add_action( 'be_title_toggle_remove', 'studio_genesis_title_toggle' );
/**
 ...
 * @return void
 */
function studio_genesis_title_toggle() {
	remove_action( 'studio_hero_section', 'studio_page_title', 10 );
	remove_action( 'studio_hero_section', 'studio_page_excerpt', 20 );
}

add_action( 'studio_hero_section', 'studio_page_title', 10 );
/**
...
function studio_page_title() {
	...
	if ( class_exists( 'WooCommerce' ) && is_shop() ) {
		...
		genesis_markup( array(
			'open'    => '<h1 %s>',
			'close'   => '</h1>',
			'content' => apply_filters( 'studio_latest_posts_title', __( 'Latest Posts', 'studio-pro' ) ),
			'context' => 'entry-title',
		) );
		...
		genesis_markup( array(
			'open'    => '<h1 %s>',
			'close'   => '</h1>',
			'content' => apply_filters( 'genesis_404_entry_title', __( 'Not found, error 404', 'studio-pro' ) ),
			'context' => 'entry-title',
		) );
...
add_action( 'studio_hero_section', 'studio_page_excerpt', 20 );
/**
...
function studio_page_excerpt() {
	...
	if ( $id && has_excerpt( $id ) ) {
		printf( '<p itemprop="description">%s</p>', do_shortcode( get_the_excerpt( $id ) ) );
	}
}

add_filter( 'genesis_attr_hero-section', 'studio_hero_section_attr' );
/**
 ...
 * @return array
 */
function studio_hero_section_attr( $attr ) {
...
add_action( 'genesis_before_content_sidebar_wrap', 'studio_hero_section' );
/**
 * Display the hero section.
 *
 * Conditionally outputs the opening and closing hero section markup and runs
 * studio_hero_section which all of our header functions are hooked to.
 *
 * @since  1.1.0
 *
 * @return void
 */
function studio_hero_section() {
	 ...
	 *
	 * @hooked studio_page_title - 10
	 * @hooked studio_page_excerpt - 20
