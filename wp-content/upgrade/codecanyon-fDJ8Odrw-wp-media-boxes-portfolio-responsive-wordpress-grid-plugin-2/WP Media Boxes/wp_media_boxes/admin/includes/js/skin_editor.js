 ...
 * ====================================================================== */      
    ...
    * ====================================================================== */    
        ...
        function set_action_fields(){
            $('.action_skin_item select.action').val( current_skin_item.find('.action').val() );
            $('.action_skin_item select.link_target').val( current_skin_item.find('.link_target').val() );
            $('.action_skin_item select.link_to').val( current_skin_item.find('.link_to').val() );
            $('.action_skin_item input.custom_url').val( current_skin_item.find('.custom_url').val());
        }
        ...
        $('body').on('click', '.save_action_skin_item', function(){
            ...
            current_skin_item.find('.custom_url').val( $('.action_skin_item input.custom_url').val() );
    ...
    * ====================================================================== */    
    ...
    /* ====================================================================== *
            EDIT ACF FIELD ON "SKIN ITEM" (POPUP)
    * ====================================================================== */    

        // init dialog/popup

        var acf_field_item = $('.acf_field_item').dialog({
            autoOpen: false,
            //height: 300,
            width: 400,
            modal: true,
        ...
        $('body').on('click', '.edit_acf_field_item', function(){
            current_skin_item   = $(this).parents('.skin_item');

            acf_field_item.find('input.acf_field').val( current_skin_item.find('.output').val() );

            acf_field_item.dialog( 'open' );
        });

        // save dialog/popup

        $('body').on('click', '.save_acf_field_item', function(){
            current_skin_item.find('.output').val( acf_field_item.find('input.acf_field').val() );
            refresh_preview();
            acf_field_item.dialog( 'close' );
        });
